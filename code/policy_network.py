...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder("float32", shape=[None, self.observation_dim])
    self.advantage_placeholder = tf.placeholder("float32", shape=[None])
    if self.discrete:
        self.action_placeholder = tf.placeholder("int32", shape=[None])
    else:
        self.action_placeholder = tf.placeholder("float32", shape=[None, 1, self.action_dim])
    #######################################################
    #########          END YOUR CODE.|CONTINUOUS ACTION CHECK          ############
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
        ...
        self.sampled_action = tf.squeeze(tf.random.categorical(action_logits, 1))
        ...
        self.dist = tfp.distributions.MultivariateNormalDiag(action_means, log_std)                
