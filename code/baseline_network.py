...
class BaselineNetwork(object):
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline_target_placeholder = tf.placeholder("float32", shape=(None, )) 
  ...
  def calculate_advantage(self, returns, observations):
    ...
    adv = returns
    adv = adv - self.sess.run(self.baseline, 
                              feed_dict={self.observation_placeholder : observations}).squeeze()              
    #######################################################
    #########          END YOUR CODE.          ############
    return adv
